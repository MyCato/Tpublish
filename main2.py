...
def get_session_file_path(force_mode: bool = False) -> str:
    ...
    while True:
        ...
        if os.path.exists(full_path):
            ...
            if confirm == 'y':
                return session_path
            # If user says no, continue the loop to ask for a different path
...
class TelethonPublisher:
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                try:
                    ...
                    print(f"💾 Corrupted config backed up as {backup_name}")
                except Exception:
                    pass
            except PermissionError:
                print(f"❌ Permission denied accessing {self.config_file}")
                print("📝 Using default configuration (changes won't be saved)")
            except Exception as e:
    ...
    def save_config(self):
        ...
        try:
            ...
            print("📝 Configuration changes won't be saved")
        except Exception as e:
    ...
    async def add_group(self):
        """Add a group by chat ID with enhanced validation"""
        print("\n📱 Add Group")
        print("=" * 30)
        ...
        while True:
            ...
            try:
                ...
                # Validate chat ID range (Telegram chat IDs are typically large numbers)
                if abs(chat_id_int) < 1000:
                    print("❌ Invalid chat ID format. Telegram chat IDs are typically much larger numbers.")
                ...
                print("❌ Chat ID must be a valid integer (e.g., -1001234567890)")
                retry = input("Would you like to try again? (y/n): ").strip().lower()
                if retry != 'y':
                    return
                continue
